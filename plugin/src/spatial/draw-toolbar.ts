...
export class DrawToolbar {
    ...
    set activeTool(value: string) {
        ...
        if (['point', 'polyline', 'polygon', 'extent', 'rectangle'].indexOf(value) > -1) {
    ...
    createTable(panel) {
        return function(queryResults) {  
            ...
            let a = queryResults.features
                        if (a.length >= 8000){
                alert('Selected polygons exceed 8000. Not all of the selection will be displayed. \n Les polygones sélectionnés dépassent 8 000. Toutes les sélections ne seront pas affichées.')
            }
    ...
    QueryLayerSIP(ext) {
        ...
        var test = parcelQueryTask.execute(parcelQuery, this.createTableSIP(this.loadingPanel))
        //console.log(test)
    }
    ...
    QueryLayerPlan(ext) {
        ...
        var test = parcelQueryTask.execute(parcelQuery, this.createTablePlan(this.loadingPanel))
        console.log(test)
    }
    ...
    QueryLayerTownship(ext) {
        ...
        var test = parcelQueryTask.execute(parcelQuery, this.createTableTownship(this.loadingPanel))
        console.log(test)
    }
    ...
    QueryLayerAdminArea(ext) {
        ...
        var test = parcelQueryTask.execute(parcelQuery, this.createTableAdminArea(this.loadingPanel))
        console.log(test)
    }
    ...
    addToMap(evt: any) {
        switch ((<any>evt).geometry.type) {
            ...
            case 'rectangle':
                // trigger observable
                this.deleteGraphics(evt.geometry)
                this.addGraphicRec(evt.geometry);
                break;
        }
    }
    ...
    addGraphic(geometry: any, symbol: any) {
        ...
        /**
     * Add graphic to graphiclayer
     * @function addGraphicRec
     * @param {Any} geometry esri geometry
     * @param {Any} symbol esri symbol
     */
    addGraphicRec(geometry: any) {
        // set color, graphic key, create the graphic, assign the key and add to layer
        this.graphicKey = Math.random().toString(36).substr(2, 9);
        const graphic = new this._bundle.Graphic(geometry);
        graphic.key = this.graphicKey;
        this.graphicsLayer.add(graphic);
